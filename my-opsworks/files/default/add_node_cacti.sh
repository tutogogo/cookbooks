...
php add_device.php --description="${nodename}" --ip="${SVR_NAME}" --template="3" --version="2" --community="dolibarr" --avail="snmp"
echo -e "CR=$?\n" 
sleep 2
...
php add_graph_template.php --graph-template-id=2 --host-id=$NODE_ID
echo -e "CR=$?\n" 
sleep 2
...
php add_graphs.php --host-id=$NODE_ID --graph-type=ds --graph-template-id=2 --snmp-query-id=1 --snmp-query-type-id=13 --snmp-field=ifName --snmp-value="eth0"
echo -e "CR=$?\n" 
sleep 2

# Disk I/O bytes/s
echo -e "Add graph Template Disk IO - Bytes per Second to node ${nodename}\n"
php add_graph_template.php  --graph-template-id=35 --host-id=$NODE_ID
echo -e "CR=$?\n"
# Add Data Query
echo -e "Add Data Query SNMP - Get Disk IO to node ${nodename} \n"
php add_graphs.php --host-id=$NODE_ID --graph-type=ds --graph-template-id=35 --snmp-query-id=10 --snmp-query-type-id=23 --snmp-field=diskIODevice --snmp-value=xvda1
echo -e "CR=$?\n"


# Add node to tree "Environnement dolibarr"
echo -e "Add node ${nodename} to default tree \n" 
php add_tree.php --type=node --node-type=host --tree-id=1 --host-id=$NODE_ID
echo -e "CR=$?\n" 
